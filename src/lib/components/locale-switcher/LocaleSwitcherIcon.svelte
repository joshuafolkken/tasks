...
<div class="relative" use:click_outside={handle_outclick}>
	<button
		...
		class="flex items-center justify-center rounded-full border border-white/20 bg-white/40 p-1.5 shadow-lg ring-1 ring-gray-900/5 backdrop-blur-md transition-all hover:bg-white/60 disabled:cursor-wait dark:border-gray-700/30 dark:bg-gray-800/40 dark:hover:bg-gray-800/60"
		aria-expanded={is_open}
		aria-label="Switch language (current: {current_locale})"
	...
	{#if is_open}
		<div
			class="absolute right-0 bottom-full z-50 mb-0.5 min-w-[5rem] rounded-lg border border-white/20 bg-white/95 p-1 shadow-xl backdrop-blur-md dark:border-gray-700/30 dark:bg-gray-800/95"
			role="menu"
		>
			{#each locales as locale (locale)}
				...
				<button
					...
					role="menuitem"
					class="flex w-full items-center justify-center rounded-md px-3 py-1.5 text-xs font-bold uppercase transition-colors {is_active
